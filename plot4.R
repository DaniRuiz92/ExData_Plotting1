...
par(mfcol = c(2, 2)) # by rows
plot(x = strptime(paste(data_set_use$Date, data_set_use$Time, sep = " "), "%d/%m/%Y %H:%M:%S"),
     y = as.numeric(as.character(data_set_use$Global_active_power)),
     type = 'l',
     xlab = "",
...
plot(x = strptime(paste(data_set_use$Date, data_set_use$Time, sep = " "), "%d/%m/%Y %H:%M:%S"),
     y = as.numeric(as.character(data_set_use$Voltage)),
     type = 'l',
     xlab = 'datetime',
     ylab = 'Voltage')

plot(x = strptime(paste(data_set_use$Date, data_set_use$Time, sep = " "), "%d/%m/%Y %H:%M:%S"),
     y = as.numeric(as.character(data_set_use$Global_reactive_power)),
     type = 'l',
     xlab = 'datetime',
